...
- A drawn diagram is a `figure` block whose `latex` is one complete tikzpicture environment. Redraw the mathematical object cleanly with named \coordinate and \draw[->]; do not trace the handwriting stroke by stroke. Map pen colour to semantic colours already defined by the document: black or blue -> mcTexte, red -> mcDef, green -> mcProp. Construction lines are dashed in mcTexte. TikZ libraries already loaded, never load one yourself: arrows.meta, calc, decorations.pathreplacing, decorations.markings, patterns, positioning, angles, quotes, intersections, shapes.geometric.
