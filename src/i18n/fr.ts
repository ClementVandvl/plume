/**
 * French — the reference dictionary.
 *
 * Keys are grouped by screen, then by element. A `.one` / `.other` pair is a
 * plural; `{name}` is filled at the call site. Adding English means copying
 * this file, translating the values, and registering it in `index.ts`.
 */
export const fr = {
  // ------------------------------------------------------------- application
  "app.name": "Plume",
  "app.loading": "Chargement…",

  "common.save": "Enregistrer",
  "common.saving": "Enregistrement…",
  "common.cancel": "Annuler",
  "common.close": "Fermer",
  "common.back": "Retour",
  "common.continue": "Continuer",
  "common.duplicate": "Dupliquer",
  "common.delete": "Supprimer",
  "common.rename": "Renommer",
  "common.openFolder": "Ouvrir le dossier",
  "common.advanced": "Avancé",
  "common.pages.one": "{count} page",
  "common.pages.other": "{count} pages",
  "common.modified": "modifié {when}",

  // --------------------------------------------------------------- title bar
  "titlebar.tools": "Outils",
  "titlebar.tools.console": "Console technique",
  "titlebar.mode.simple": "Simple",
  "titlebar.mode.advanced": "Avancé",
  "titlebar.minimize": "Réduire",
  "titlebar.maximize": "Agrandir ou restaurer",
  "titlebar.close": "Fermer la fenêtre",

  // -------------------------------------------------------------- navigation
  "nav.section": "Mon classeur",
  "nav.home": "Accueil",
  "nav.documents": "Mes documents",
  "nav.houseStyle": "Mes chartes",
  "nav.instructions": "Mes consignes",
  "nav.trash": "Corbeille",
  "nav.help": "Aide",
  "nav.settings": "Réglages",
  "nav.settings.missing": "Prérequis manquant",

  // ------------------------------------------------------------------ status
  "boot.title": "Plume prépare votre classeur",
  "boot.hint": "Lecture de vos documents et de vos chartes…",
  "status.draft": "Pas encore lu",
  "status.review": "À vérifier",
  "status.ready": "PDF prêt",
  "status.reading": "Lecture en cours",

  // ------------------------------------------------------------------- steps
  "steps.pages": "Photos",
  "steps.read": "Lecture",
  "steps.work": "Travail sur le document",
  "steps.review": "Relecture",
  "steps.export": "PDF",

  // ------------------------------------------------------------------- home
  "home.greeting": "Bonjour",
  "home.subtitle.review": "Vous avez un document en cours de relecture.",
  "home.subtitle.reading": "Une lecture est prête à être lancée.",
  "home.subtitle.idle": "Photographiez une page, Plume la réécrit dans votre charte.",
  "home.resume.overline": "Reprendre",
  "home.resume.summary": "Plume a lu vos {pages} pages. Il reste {remaining} sur {blocks}.",
  "home.resume.left.one": "{count} passage à vérifier",
  "home.resume.left.other": "{count} passages à vérifier",
  "home.resume.progress": "{reviewed} sur {blocks} relus",
  "home.resume.continue": "Continuer la relecture",
  "home.resume.open": "Voir le document",
  "home.engine.banner":
    "Le moteur de mise en page n'est pas encore installé. Sans lui, Plume peut lire vos pages mais pas fabriquer le PDF.",
  "home.engine.install": "Installer maintenant",
  "home.drop.title": "Nouveau document",
  "home.drop.text": "Glissez les photos de vos pages ici, ou {browse}. L'ordre des photos donne l'ordre des pages.",
  "home.drop.browse": "parcourez vos fichiers",
  "home.howto.title": "Comment ça marche",
  "home.howto.step1": "Vous photographiez vos pages.",
  "home.howto.step2": "Plume les lit et les remet dans votre charte.",
  "home.howto.step3": "Vous vérifiez les passages dont elle n'est pas sûre.",
  "home.howto.step4": "Vous obtenez le PDF, version prof ou version élève.",
  "home.others.title": "Vos autres documents",
  "home.others.all": "Tout voir",

  // -------------------------------------------------------- first launch
  "welcome.title": "Bienvenue dans Plume",
  "welcome.text":
    "Photographiez vos pages manuscrites. Plume les lit, les remet dans votre charte, et vous rend un PDF prof et un PDF élève.",
  "welcome.check.title": "Vérification de votre machine",
  "welcome.drop.title": "Déposez les photos de votre premier document",
  "welcome.drop.browse": "ou {browse}",
  "welcome.engine.hint":
    "Vous pouvez commencer sans le moteur de mise en page : la lecture fonctionne déjà.",

  // ----------------------------------------------------------------- documents
  "documents.title": "Mes documents",
  "documents.search": "Rechercher un document",
  "documents.new": "Nouveau document",
  "documents.import": "Importer",

  // Ce qu'est un document, dans les mots du professeur.
  "tags.title": "Étiquettes",
  "tags.subtitle": "Ce qu'est « {title} », dans vos mots",
  "tags.add.placeholder": "cours, exercices, DS…",
  "tags.add": "Ajouter",
  "tags.known": "Déjà utilisées",
  "tags.remove": "Retirer « {tag} »",
  "tags.empty": "Aucune étiquette : ce document n'apparaîtra que dans « Toutes ».",

  // Un document écrit ailleurs — une fiche demandée à Claude, par exemple — qui
  // arrive sous forme de passages et devient un document comme un autre.
  "import.title": "Importer un document",
  "import.subtitle.paste": "Collez le document, ou choisissez un fichier",
  "import.subtitle.check": "Vérifiez avant de l'ajouter au classeur",
  "import.explain":
    "Plume attend une liste de passages, pas un document LaTeX. Copiez les instructions ci-dessous, collez-les à Claude avec votre demande, puis rapportez sa réponse ici.",
  "import.copy": "Copier les instructions pour Claude",
  "import.copied": "Instructions copiées",
  "import.pick": "Choisir un fichier…",
  "import.paste": "Le document, en JSON",
  "import.paste.placeholder": "{ \"title\": \"Fiche d'exercices\", \"blocks\": [ … ] }",
  "import.reading": "Lecture…",
  "import.name": "Titre du document",
  "import.name.placeholder": "Fiche d'exercices — Vecteurs",
  "import.charte": "Charte",
  "import.commit": "Ajouter au classeur",
  "import.importing": "Import…",
  "import.passages.one": "1 passage",
  "import.passages.other": "{count} passages",
  "import.tags": "Étiquettes",
  "import.tags.placeholder": "exercices, seconde",
  "import.tags.hint": "Séparées par des virgules. Elles servent à trier le classeur.",
  "import.warnings.one": "1 point à regarder",
  "import.warnings.other": "{count} points à regarder",
  "documents.filter.all": "Tous",
  "documents.column.document": "Document",
  "documents.column.state": "Où en est-on",
  "documents.column.next": "Prochaine étape",
  "documents.state.doubtful.one": "{count} passage à vérifier",
  "documents.state.doubtful.other": "{count} passages à vérifier",
  "documents.state.ready": "Tout est relu, PDF prêt",
  "documents.state.reviewed": "Tout est relu",
  "documents.state.unread": "Photos importées, lecture non lancée",
  "documents.state.empty": "Aucune photo pour l'instant",
  "documents.action.review": "Continuer la relecture",
  "documents.action.reread": "Relire",
  "documents.action.watch": "Voir la lecture",
  "documents.action.openPdf": "Ouvrir le PDF",
  "documents.action.read": "Lire les pages",
  "documents.action.addPages": "Ajouter des photos",
  "documents.menu.label": "Autres actions",
  "documents.menu.open": "Ouvrir le document",
  "documents.menu.rename": "Renommer",
  "documents.menu.tags": "Étiquettes…",
  "documents.menu.trash": "Mettre à la corbeille",
  "documents.tags.label": "Étiquettes",
  "documents.empty.none": "Aucun document pour l'instant.",
  "documents.empty.filtered": "Aucun document ne correspond à cette recherche.",

  // ---------------------------------------------------------------- document
  "document.back": "Mes documents",
  "document.rename.title": "Renommer le document",
  "document.rename.message":
    "Le dossier sur le disque garde son nom ; seul le titre affiché change.",
  "document.rename.field": "Titre",
  "document.rename.confirm": "Renommer",
  "document.trash.title": "Mettre « {title} » à la corbeille ?",
  "document.trash.message":
    "Le document est retiré de la liste mais reste sur votre disque, avec ses photos et ses PDF. Vous pourrez le restaurer depuis la corbeille.",
  "document.trash.confirm": "Mettre à la corbeille",

  // ----------------------------------------------------------- document/photos
  "pages.title.one": "{count} page dans ce document",
  "pages.title.other": "{count} pages dans ce document",
  "viewer.title": "Photo de la page {number}",
  "viewer.of": "Page {number} sur {total}",
  "viewer.zoom": "Taille réelle",
  "viewer.fit": "Ajuster à l'écran",
  "viewer.previous": "Page précédente",
  "viewer.next": "Page suivante",
  "pages.reorder.hint":
    "Glissez une vignette pour changer l'ordre des pages.",
  "pages.add": "+ Ajouter des photos",
  "pages.page": "Page {number}",
  "pages.remove.title": "Retirer la page {number} ?",
  "pages.remove.message":
    "La photo est supprimée du document. Votre photothèque n'est pas touchée.",
  "pages.remove.detail":
    "La transcription de cette page est supprimée et les pages suivantes sont renumérotées. Les autres pages sont conservées.",
  "pages.remove.confirm": "Retirer la page",
  "pages.remove.aria": "Retirer la page {number}",
  "pages.rules.title": "Vos consignes pour ce document",
  "pages.rules.hint":
    "Dites-le comme à un collègue qui recopie vos pages. Vos mots passent avant les règles générales.",
  "pages.rules.placeholder":
    "Ce que je surligne en orange doit être en gras. Un trait bleu en marge veut dire : à garder pour moi seulement.",
  "pages.rules.save": "Enregistrer mes consignes",
  "pages.rules.examples": "Exemples à insérer",
  "pages.rules.example1": "Encadré = définition",
  "pages.rules.example1.text": "Ce qui est encadré est une définition.",
  "pages.rules.example2": "Étoile = à ne pas transcrire",
  "pages.rules.example2.text": "Ce qui est marqué d'une étoile ne doit pas être transcrit.",
  "pages.rules.example3": "Écrit au crayon = brouillon",
  "pages.rules.example3.text": "Ce qui est écrit au crayon est du brouillon, à ignorer.",
  "pages.rules.global":
    "Les consignes valables pour tous vos documents se règlent dans {page}.",
  "pages.start.one": "Lancer la lecture de la page",
  "pages.start.other": "Lancer la lecture des {count} pages",

  // ------------------------------------------------------------- document/read
  "read.running.title": "Plume lit vos pages",
  "read.idle.title": "Lecture des pages",
  "read.progress": "Page {done} sur {total}",
  "read.stop": "Arrêter la lecture",
  "read.reassurance":
    "Vous pouvez changer d'écran : la lecture continue, et le document est marqué « lecture en cours » dans vos listes.",
  "read.list.title": "Page par page",
  "read.state.done.one": "Lue — {count} passage trouvé",
  "read.state.done.other": "Lue — {count} passages trouvés",
  "read.state.waiting": "En attente",
  "read.state.reading": "Lecture en cours…",
  "read.state.failed": "Échec de lecture",
  "read.state.cancelled": "Annulée — pages déjà lues conservées",
  "read.quality.title": "Qualité de lecture",
  "read.quality.fast": "Rapide",
  "read.quality.fast.hint": "Recommandé. Quelques secondes par page.",
  "read.quality.deep": "Approfondie",
  "read.quality.deep.hint": "Plus lente, meilleure sur les schémas.",
  "read.advanced.title": "Détails techniques",
  "read.advanced.hint":
    "Modèle, coût, pages lues en parallèle et journal de lecture s'affichent quand vous passez en mode {mode}, en haut de la fenêtre.",
  "read.model.label": "Modèle de lecture",
  "read.start": "Lire les pages",
  "read.restart": "Relire les pages",
  "read.running": "Lecture en cours…",
  "read.reread.title": "Relire toutes les pages ?",
  "read.reread.message":
    "La transcription actuelle ({blocks} passages) est remplacée, avec vos corrections manuelles, vos confirmations et vos annotations. Elle reste dans les versions du document : vous pourrez y revenir.",
  "read.reread.detail.one": "{count} page sera relue et consommera du quota.",
  "read.reread.detail.other": "{count} pages seront relues et consommeront du quota.",
  "read.reread.confirm": "Relire et remplacer",
  "read.done": "Lecture terminée — {blocks} passages, {cost}.",
  "read.cancelled":
    "Lecture annulée — {blocks} passages conservés sur les pages déjà lues, {cost} dépensés.",
  "read.failedPage": "Page {page} : {message}",
  "read.pageProgress": "Page {page} sur {total} — {cost} cumulés.",

  // ----------------------------------------------------------- document/review
  "review.filter.doubt": "À vérifier",
  "review.filter.all": "Tout le document",
  "review.filter.teacher": "Réservé au prof",
  "review.filter.student": "Réservé aux élèves",
  "review.filter.hidden": "Mis de côté",
  "review.makePdf": "Fabriquer le PDF",
  "review.unread": "Ce document n'a pas encore été lu. Revenez à l'étape « Lecture ».",
  "review.correct.one": "Corriger {count} passage",
  "review.correct.other": "Corriger {count} passages",
  "review.correct.stop": "Annuler la correction",
  "review.correct.done": "Correction terminée — {done} sur {total}.",
  "review.correct.cancelled.one":
    "Corrections annulées — {done} sur {total} appliquée, conservée.",
  "review.correct.cancelled.other":
    "Corrections annulées — {done} sur {total} appliquées, conservées.",
  "review.correct.failed": "{block} : {message}",
  "review.correct.progress": "Passage {done} sur {total} corrigé.",
  "review.correct.running": "Plume reprend vos passages",
  "review.correct.finished": "Correction terminée",
  "review.correct.stopped": "Correction interrompue",
  "review.correct.at": "Passage {done} sur {total} — {name}",
  "review.correct.name": "{kind}, page {page}",

  "chat.open": "Demander à Claude",
  "chat.title": "Demander à Claude",
  "chat.subtitle": "Sur tout le document",
  "chat.tab.talk": "Discussion",
  "chat.tab.versions": "Versions",
  "chat.empty.title": "Une demande pour tout le document",
  "chat.empty.text":
    "Claude lit le document entier et modifie les passages concernés. Avant chaque modification, Plume garde l'état précédent : vous pouvez toujours revenir en arrière.",
  "chat.example.grid": "Mets les définitions côte à côte, sur deux colonnes",
  "chat.example.numbers": "Numérote les parties A, B, C au lieu de I, II, III",
  "chat.example.teacher": "Réserve toutes les démonstrations à ma version",
  "chat.example.center": "Centre toutes les figures",
  "chat.placeholder": "Que voulez-vous changer dans le document ?",
  "chat.send": "Envoyer",
  "chat.hint": "Entrée pour envoyer, Maj + Entrée pour aller à la ligne.",
  "chat.stop": "Arrêter",
  "chat.working": "Claude travaille…",
  "chat.busy": "Une lecture ou une correction est en cours : attendez qu'elle se termine.",
  "chat.you": "Vous",
  "chat.claude": "Claude",
  "chat.tally.none": "Document inchangé",
  "chat.tally.edited.one": "{count} passage modifié",
  "chat.tally.edited.other": "{count} passages modifiés",
  "chat.tally.added.one": "{count} ajouté",
  "chat.tally.added.other": "{count} ajoutés",
  "chat.tally.removed.one": "{count} retiré",
  "chat.tally.removed.other": "{count} retirés",
  "chat.undo": "Annuler cette modification",
  "chat.undo.title": "Annuler la dernière modification de Claude ?",
  "chat.undo.message": "Le document revient à son état d'avant « {label} ».",
  "chat.undo.detail":
    "Les retouches faites à la main depuis sont annulées aussi. L'état actuel n'est pas perdu : il reste dans les versions.",
  "chat.undo.confirm": "Annuler la modification",
  "chat.clear": "Effacer la discussion",
  "chat.clear.title": "Effacer la discussion ?",
  "chat.clear.message":
    "Claude oublie les échanges précédents. Le document et ses versions ne changent pas.",
  "chat.clear.confirm": "Effacer",

  "versions.hint":
    "Plume garde les trois derniers états du document, pris juste avant que Claude le modifie — par une demande, une correction ou une relecture. Jamais pour vos retouches à la main.",
  "versions.empty": "Aucune version pour l'instant : la première sera prise à la prochaine modification par Claude.",
  "versions.kind.chat": "Avant votre demande",
  "versions.kind.corrections": "Avant les corrections",
  "versions.kind.reading": "Avant la relecture des photos",
  "versions.kind.restore": "Avant l'annulation",
  "versions.restoreOf": "de « {label} »",
  "versions.blocks.one": "{count} passage",
  "versions.blocks.other": "{count} passages",
  "versions.restore": "Revenir à cette version",
  "versions.restore.title": "Revenir à cette version ?",
  "versions.restore.message": "Le document reprend son état « {kind} » ({when}).",
  "versions.restore.detail":
    "L'état actuel n'est pas perdu : il prend la place de cette version dans la liste, et vous pourrez y revenir.",
  "versions.stale": "Les photos ont changé depuis : cette version ne peut plus être remise.",

  "panel.position": "Passage {index} sur {total}",
  "panel.meta": "{kind} · page {page}",
  "panel.previous": "Passage précédent",
  "panel.next": "Passage suivant",
  "panel.photo.title": "Votre page",
  "panel.photo.enlarge": "Agrandir la photo",
  "panel.wrong.title": "Qu'est-ce qui ne va pas ?",
  "panel.wrong.formula": "Formule fausse",
  "panel.wrong.kind": "Mauvais type de passage",
  "panel.wrong.missing": "Texte manquant",
  "panel.wrong.teacher": "Réserver au prof",
  "panel.note.hint": "Écrivez comme vous le diriez à voix haute. Plume ne reprend que ce passage.",
  "panel.note.placeholder":
    "La flèche va de M vers N, pas l'inverse. Et c'est une propriété, pas un théorème.",
  "panel.audience.title": "Ce passage apparaît dans",
  "panel.audience.teacher": "Ma version",
  "panel.audience.student": "Version élève",
  "panel.hidden.title": "Dans le PDF",
  "panel.hidden.label": "Mettre de côté",
  "panel.hidden.hint":
    "Le passage reste dans le document mais sort de tous les PDF. Rien n'est supprimé : il suffit de rebasculer pour le remettre.",
  "panel.kind.label": "Type de passage",
  "panel.title.label": "Titre (facultatif)",
  "copy.action": "Copier vers un autre document",
  "copy.title": "Copier le passage",
  "copy.subtitle": "Il reste ici, et une copie se place à la toute fin de l'autre document.",
  "copy.tab.existing": "Document existant",
  "copy.tab.new": "Nouveau document",
  "copy.search": "Rechercher un document ou une étiquette",
  "copy.loading": "Chargement du classeur…",
  "copy.none": "Aucun autre document ne peut le recevoir : un document photographié doit d'abord être lu.",
  "copy.none.match": "Aucun document ne correspond.",
  "copy.reading": "Lecture en cours — disponible quand elle sera finie",
  "copy.count.one": "{count} passage",
  "copy.count.other": "{count} passages",
  "copy.existing.hint": "La copie s'ajoute après le dernier passage du document choisi.",
  "copy.existing.action": "Copier à la fin",
  "copy.new.title": "Titre du nouveau document",
  "copy.new.placeholder": "Exercices — vecteurs",
  "copy.new.hint":
    "Un document vierge, sans photo à lire, dans la même charte et avec les mêmes étiquettes. La copie en sera le premier passage.",
  "copy.new.action": "Créer et copier",
  "copy.copying": "Copie…",
  "copy.done": "Passage copié dans « {title} ».",
  "copy.open": "Ouvrir",
  "insert.title": "Ajouter un passage",
  "insert.here": "Ajouter ici",
  "insert.first": "Ajouter au début",
  "insert.kind.headings": "Titres — sans LaTeX",
  "insert.kind.passages": "Passages",
  "insert.heading.title": "Texte du titre",
  "insert.heading.placeholder": "Calcul littéral",
  "insert.heading.hint":
    "Un titre n'a pas de contenu LaTeX : la charte le compose à partir de son texte et de son numéro.",
  "insert.number": "Numéro",
  "insert.number.placeholder": "3, II, a…",

  // Où la classe en est. Une marque sur le document, posée après la séance, que
  // chaque fabrication relit ensuite.
  "taught.here": "La classe s'arrête ici",
  "taught.boundary": "Vu en classe jusqu'ici",
  "taught.clear": "retirer",
  "taught.unnamed": "le passage « {kind} » de la page {page}",
  "taught.card.heading": "Vu jusqu'à « {heading} »",
  "taught.card.count.one": "Vu en classe : 1 passage",
  "taught.card.count.other": "Vu en classe : {count} passages",
  "insert.tab.latex": "Écrire le LaTeX",
  "insert.tab.photo": "Photographier",
  "insert.kind": "Type de passage",
  "insert.name": "Titre (facultatif)",
  "insert.name.placeholder": "vecteurs colinéaires",
  "insert.latex": "Contenu LaTeX",
  "insert.latex.placeholder":
    "Deux vecteurs $\\vec{u}$ et $\\vec{v}$ sont colinéaires s'il existe…",
  "insert.preview": "Aperçu",
  "insert.add": "Ajouter le passage",
  "insert.photo.hint":
    "La photo rejoint les pages du document, juste après celle où vous ajoutez, et Plume la lit comme les autres. Elle apparaîtra dans l'onglet Photos.",
  "insert.photo.pick": "Choisir une photo",
  "insert.photo.reading": "Lecture en cours…",
  "panel.align.title": "Alignement",
  "panel.align.default": "Charte",
  "panel.align.left": "Gauche",
  "panel.align.center": "Centré",
  "panel.align.right": "Droite",
  "panel.align.hint":
    "Déplace le passage sans toucher à son contenu. Une formule sur sa propre ligne garde le centrage de LaTeX.",
  "panel.delete": "Supprimer le passage",
  "panel.delete.title": "Supprimer ce passage ?",
  "panel.delete.message":
    "Il disparaît de la transcription. La photo de la page n'est pas touchée.",
  "panel.delete.detail":
    "Une nouvelle lecture de la page le ferait revenir.",
  "split.action": "Scinder le passage",
  "split.why":
    "Pour traiter séparément les deux moitiés — garder une correction pour vous, par exemple.",
  "split.hint": "Choisissez où couper. Le passage devient deux passages.",
  "split.here": "Couper ici",
  "split.tooShort":
    "Ce passage est d'un seul tenant : il n'a pas de coupure évidente. Ajoutez une ligne vide dans son LaTeX à l'endroit voulu, puis réessayez.",
  "panel.latex.title": "Modifier le code LaTeX",
  "panel.latex.label": "Contenu LaTeX",
  "panel.confirm": "✓ C'est correct",
  "panel.fix": "Corriger ce passage",
  "panel.save": "Enregistrer",
  "panel.shortcuts": "Raccourci : Entrée pour valider, ↓ pour le passage suivant.",

  // ----------------------------------------------------------- document/export
  "export.choice.title": "Quelle version voulez-vous ?",
  "export.teacher.title": "Version professeur",
  "export.teacher.hint.one": "Le passage, commentaires compris.",
  "export.teacher.hint.other": "Les {count} passages, commentaires compris.",
  "export.student.title": "Version élève",
  "export.student.hint.one": "{count} passage · {removed} retiré(s), réservé(s) à vous.",
  "export.student.hint.other": "{count} passages · {removed} retirés, réservés à vous.",
  "export.student.hint.same": "Identique à la version professeur pour l'instant.",
  "export.all.title": "Version complète",
  "export.all.hint": "Tous les passages, sans distinction de version.",
  "export.hidden.one": "1 passage mis de côté n'apparaît dans aucune version.",
  "export.hidden.other": "{count} passages mis de côté n'apparaissent dans aucune version.",
  "export.reach.title": "Jusqu'où ?",
  "export.reach.taught": "Jusqu'où la classe en est",
  "export.reach.taught.hint.one": "1 passage sur {total} — s'arrête après {last}.",
  "export.reach.taught.hint.other": "{count} passages sur {total} — s'arrête après {last}.",
  "export.reach.whole": "Tout le document",
  "export.reach.whole.hint.one": "Le passage, du début à la fin.",
  "export.reach.whole.hint.other": "Les {count} passages, du début à la fin.",
  "export.reach.unmarked":
    "Aucun point d'arrêt n'est posé. Dans la relecture, marquez le dernier passage vu en classe.",
  "export.reach.dangling":
    "Le dernier passage retenu est un titre : le PDF se terminerait sur un titre sans contenu.",
  "export.reach.complete": "Le point d'arrêt est sur le dernier passage : le PDF sera complet.",
  "export.sheet.title": "Sur la feuille",
  "export.sheet.1": "1 page",
  "export.sheet.2": "2 pages",
  "export.sheet.4": "4 pages",
  "export.sheet.hint.1": "Le PDF tel quel, une page par feuille A4.",
  "export.sheet.hint.2": "Deux pages côte à côte sur un A4 en paysage, chacune réduite de moitié.",
  "export.sheet.hint.4": "Quatre pages en grille sur un A4, chacune réduite au quart.",
  "export.sheet.max": "Maximiser",
  "export.sheet.hint.max":
    "La fiche est recomposée dans la plus petite case où elle tient entière — le texte réduit au besoin, jamais sous 6 pt — et posée autant de fois que la feuille en contient.",
  "export.sheet.repeat": "Répéter la même page dans chaque case",
  "export.sheet.repeat.on": "Pour imprimer une fiche en plusieurs exemplaires et la découper.",
  "export.sheet.repeat.off": "Les pages se suivent, d'une case à l'autre.",
  "export.partial.note": "Fabrication partielle : elle n'écrase pas le PDF complet du document.",
  "export.build": "Fabriquer le PDF",
  "export.building": "Fabrication…",
  "export.ready": "PDF prêt",
  "export.openPdf": "Ouvrir le PDF",
  "export.reveal": "Voir le dossier",
  "export.texRow": "Fichier source .tex et journal de compilation",
  "export.noTex": "Fabriquez d'abord le PDF : le fichier .tex apparaîtra ici.",
  "figure.compiling": "Compilation du schéma…",
  "figure.alt": "Schéma du document",
  "passage.compiling": "Mise en page par le moteur…",
  "passage.alt": "Passage tel qu'il sera imprimé",
  "export.openTex": "Ouvrir le .tex",
  "export.failed": "Le .tex est écrit, mais la compilation a échoué : {error}",
  "export.preview.title": "Aperçu — {audience}",
  "export.sameVersions":
    "Aucun passage n'est réservé au professeur : les versions sont identiques. Marquez un passage « Ma version » pendant la relecture, ou décrivez votre convention dans vos annotations.",

  // ----------------------------------------------------------------- layout
  "houseStyle.title": "Mes chartes",
  "houseStyle.subtitle.builtin": "Charte livrée avec Plume",
  "houseStyle.subtitle.copy": "Copie de « {name} »",
  "houseStyle.usedBy.one": "utilisée par {count} document",
  "houseStyle.usedBy.other": "utilisée par {count} documents",
  "houseStyle.builtin.notice":
    "Modèle livré avec Plume. Ses couleurs et ses règles vous appartiennent : vos modifications survivent aux mises à jour. Son squelette LaTeX, lui, est remplacé à chaque nouvelle version — dupliquez-le pour en changer la structure.",
  "houseStyle.tab.keys": "Apparence",
  "houseStyle.tab.rules": "Règles",
  "houseStyle.tab.preamble": "Préambule",
  "houseStyle.tab.blocks": "Blocs",
  "houseStyle.size.unit": "pt",
  "houseStyle.identity.title": "Identité",
  "houseStyle.identity.name": "Nom",
  "houseStyle.identity.description": "Description",
  "houseStyle.saved": "Charte enregistrée.",
  "houseStyle.compiles": "La charte compile.",
  "houseStyle.verify": "Vérifier",
  "houseStyle.verifying": "Compilation…",
  "houseStyle.preview.substituted": "Voir valeurs substituées",
  "houseStyle.preview.refresh": "Rafraîchir le rendu",
  "houseStyle.preamble.hint":
    "Les {syntax} sont remplacés par les valeurs de l'onglet Apparence. « Vérifier » compile la charte seule : une erreur se voit ici plutôt qu'au moment d'exporter un document.",
  "houseStyle.blocks.title": "Écriture des blocs",
  "houseStyle.blocks.hint":
    "Comment chaque type de passage reconnu est écrit en LaTeX. Un passage sans correspondance sort tel quel, sans son environnement.",
  "houseStyle.mode.command": "Commande — \\nom{contenu}",
  "houseStyle.mode.environment": "Environnement — \\begin{nom}…",
  "houseStyle.mode.raw": "Brut — le LaTeX du bloc tel quel",
  "houseStyle.mode.centered": "Centré — dans un center",
  "houseStyle.rules.hint":
    "Ce que cette charte attend du LaTeX produit. Elles s'ajoutent à vos consignes générales : celles-là valent partout, celles-ci suivent la charte.",
  "houseStyle.rules.add": "+ Ajouter une règle",
  "houseStyle.rules.empty":
    "Aucune règle. Par exemple : aligner sur le signe égal les lignes de calcul qui commencent par « = ».",
  "houseStyle.advanced.row": "Préambule LaTeX et correspondance des blocs",
  "houseStyle.duplicating": "Duplication…",
  "houseStyle.duplicate.title": "Dupliquer la charte",
  "houseStyle.duplicate.message":
    "Une copie indépendante de « {name} », que vous pourrez modifier entièrement.",
  "houseStyle.duplicate.detail": "L'original n'est pas touché.",
  "houseStyle.duplicate.field": "Nom de la nouvelle charte",
  "houseStyle.duplicate.default": "{name} (copie)",
  "houseStyle.delete.title": "Supprimer « {name} » ?",
  "houseStyle.delete.message": "Les documents qui l'utilisent devront en choisir un autre.",
  "houseStyle.delete.detail": "La charte part à la corbeille du classeur, elle n'est pas effacée.",
  "houseStyle.danger.title": "Zone sensible",
  "houseStyle.danger.text": "Supprimer « {name} » — la charte part à la corbeille du classeur.",
  "houseStyle.none": "Aucune charte installée.",
  "houseStyle.builtinFlag": "livré",

  // ------------------------------------------------------------- consignes
  "instructions.title": "Mes consignes",
  "instructions.subtitle":
    "Ce que Plume doit savoir avant de lire, et qui vaut pour tous vos documents. Une consigne propre à un seul document se règle dans le document.",
  "instructions.tab.marks": "Mes annotations",
  "instructions.tab.conventions": "Mes consignes",
  "instructions.marks.about":
    "Ce que vos marques au feutre veulent dire. Plume les repère sur la photo et applique ce que vous avez décidé : un surlignage devient du gras, un trait en marge réserve un passage à votre version.",
  "instructions.conventions.about":
    "Des consignes générales, transmises à chaque lecture. Contrairement aux annotations, elles ne dépendent d'aucune marque sur la feuille : ce sont vos habitudes de rédaction, dites une fois pour toutes — la façon de dessiner les schémas, de nommer les points, d'abréger un intitulé.",
  "instructions.marks.add": "+ Ajouter une marque",
  "instructions.marks.empty":
    "Aucune marque. Par exemple : surligné orange → gras, trait bleu en marge → réservé au professeur.",
  "instructions.sentence": "Quand je vois {trigger}, Plume applique : {effect}.",
  "instructions.conventions.add": "+ Ajouter une consigne",
  "instructions.conventions.empty":
    "Aucune consigne. Par exemple : jamais d'annotation par-dessus un trait dans les schémas.",
  "instructions.disabled": "désactivée",
  "instructions.delete.rule.title": "Supprimer cette marque ?",
  "instructions.delete.rule.message": "Elle ne sera plus appliquée à vos prochaines lectures.",
  "instructions.delete.convention.title": "Supprimer cette consigne ?",
  "instructions.delete.convention.message":
    "Elle ne sera plus transmise au modèle lors de vos prochaines lectures.",
  "instructions.delete.detail":
    "Pour la désactiver sans la perdre, décochez « Appliquer » à la place.",

  "rulePanel.title": "Règle de lecture",
  "rulePanel.active": "active",
  "rulePanel.apply": "Appliquer cette règle",
  "rulePanel.when": "Quand je vois",
  "rulePanel.mark": "Marque",
  "rulePanel.colour": "Couleur",
  "rulePanel.colourName": "Nom de la couleur",
  "rulePanel.colourName.placeholder": "orange",
  "rulePanel.colourName.hint":
    "Le nom aide autant que le code : une photo rend rarement la couleur exacte.",
  "rulePanel.describe": "Décrivez la marque",
  "rulePanel.describe.placeholder": "Une accolade tracée à gauche de plusieurs lignes.",
  "rulePanel.then": "Alors",
  "rulePanel.effect": "Effet",
  "rulePanel.blockKind": "Type de passage",
  "rulePanel.customEffect": "Décrivez l'effet",
  "rulePanel.customEffect.placeholder": "Transcrire ce passage en petites capitales.",

  "conventionPanel.title": "Consigne",
  "conventionPanel.apply": "Appliquer cette consigne",
  "conventionPanel.name": "Nom",
  "conventionPanel.name.placeholder": "Annotations des schémas",
  "conventionPanel.name.hint":
    "Pour vous y retrouver dans la liste. Transmis au modèle en tête de la consigne.",
  "conventionPanel.text": "Consigne",
  "conventionPanel.text.placeholder":
    "Aucune étiquette ne doit toucher ni chevaucher un trait, une flèche, un point ou une autre étiquette…",
  "conventionPanel.text.hint":
    "Reprise mot pour mot. Écrivez-la comme vous l'expliqueriez à un collègue.",

  // ------------------------------------------------------------------ trash
  "trash.title": "Corbeille",
  "trash.subtitle":
    "Ces documents sont retirés de la liste mais toujours sur votre disque, avec leurs photos et leurs PDF. Plume ne supprime un fichier que sur votre ordre, ici.",
  "trash.empty": "La corbeille est vide.",
  "trash.meta": "{pages} · mis à la corbeille {when}",
  "trash.restore": "Restaurer",
  "trash.purge": "Supprimer définitivement",
  "trash.purge.title": "Supprimer « {title} » définitivement ?",
  "trash.purge.message.one":
    "La photo, la transcription et les fichiers produits seront effacés de votre disque. Cette action ne peut pas être annulée.",
  "trash.purge.message.other":
    "Les {count} photos, la transcription et les fichiers produits seront effacés de votre disque. Cette action ne peut pas être annulée.",
  "trash.purge.confirm": "Supprimer définitivement",
  "trash.purge.keep": "Garder le document",

  // --------------------------------------------------------------- settings
  "settings.title": "Réglages",
  "settings.subtitle": "Plume travaille sur votre machine. Rien n'est envoyé ailleurs.",
  "settings.needs.title": "Ce dont Plume a besoin",
  "settings.tool.ready": "Prêt",
  "settings.tool.install": "Installer",
  "settings.tool.installing": "Préparation…",
  "settings.tool.installExternal": "Installer {name} ↗",
  "settings.tool.login": "Se connecter…",
  "settings.tool.relogin": "Se reconnecter",
  "settings.tool.loginPending": "Autorisez Plume dans le navigateur qui s'ouvre…",
  "settings.tool.connected": "Connecté — {email}",
  "settings.tool.disconnected": "Claude n'est pas connecté : rien ne peut être lu.",
  "settings.tool.loginHint":
    "Un terminal s'ouvre avec la commande déjà écrite ; votre navigateur demande une autorisation. Rien à taper.",

  // Une session qui a expiré : la seule panne que l'interface peut réparer.
  "auth.banner": "Claude n'est pas connecté : Plume ne peut pas lire vos pages.",
  "auth.login": "Se connecter…",
  "auth.pending": "Autorisez Plume dans le navigateur qui s'ouvre…",
  "auth.failed.title": "Claude doit être reconnecté",
  "auth.failed.text":
    "Sa session a expiré. Reconnectez-le, puis relancez la lecture : les pages déjà lues sont conservées.",
  "auth.retry": "Relancer la lecture",
  "settings.appearance.title": "Apparence",
  "settings.theme.light": "Clair",
  "settings.theme.dark": "Sombre",
  "settings.theme.system": "Comme mon système",
  "settings.general.title": "Général",
  "settings.updates.title": "Prévenir quand une mise à jour existe",
  "settings.updates.hint":
    "Pour Plume comme pour Claude Code. L'installation demande toujours votre accord.",
  "settings.mcp.title": "Connexion MCP",
  "settings.mcp.hint": "Claude pourra vous proposer d'envoyer un document dans Plume.",
  "settings.mcp.install": "Installer dans Claude…",
  "settings.mcp.saved": "Extension enregistrée",
  "settings.mcp.copy": "Copier le réglage",
  "settings.mcp.copied": "Réglage copié",
  "settings.mcp.steps.bundle":
    "Dans Claude : Réglages → Extensions → Paramètres avancés → Installer une extension…, puis choisissez plume.mcpb. Si Claude s'est ouvert de lui-même, c'est déjà fait.",
  "settings.mcp.steps.config":
    "Dans Claude : Réglages → Développeur → Modifier la configuration, puis collez le réglage dans claude_desktop_config.json. Si le fichier liste déjà des serveurs, ajoutez l'entrée « plume » dans « mcpServers ». Relancez Claude.",
  "settings.folder.title": "Mes documents sont rangés dans",
  "settings.advanced.row": "Modèle de lecture, pages en parallèle, journal technique",
  "settings.advanced.title": "Lecture",
  "settings.model.label": "Modèle par défaut",
  "settings.model.hint": "Modifiable document par document au moment de la lecture.",
  "settings.parallel.label": "Pages lues en parallèle",
  "settings.parallel.auto": "Automatique — selon la mémoire de la machine",
  "settings.parallel.autoAt":
    "Automatique — {pages} à la fois ({memory} Go détectés)",
  "settings.parallel.autoUnknown":
    "Automatique — {pages} à la fois (mémoire non détectée)",
  "settings.parallel.one": "1 — machine modeste (8 Go)",
  "settings.parallel.three": "3 — machine confortable (16 Go et plus)",
  "settings.parallel.hint":
    "Chaque page lue lance son propre processus Claude, qui pèse plusieurs centaines de Mo. Trop large, la machine sature et se fige.",
  "settings.version": "Version {version}",
  "settings.done": "Terminé",

  "model.sonnet": "Sonnet — rapide, recommandé",
  "model.opus": "Opus — plus lent, plus fin",
  "model.fable": "Fable",

  // ---------------------------------------------------------------- updates
  "claudeUpdate.name": "Claude Code {version}",
  "claudeUpdate.idle": "Pas encore vérifié.",
  "claudeUpdate.checking": "Recherche en cours…",
  "claudeUpdate.current": "Claude Code est à jour.",
  "claudeUpdate.updated": "Mis à jour en version {version}.",
  "claudeUpdate.available": "Version {version} disponible.",
  "claudeUpdate.availableBehind":
    "Version {version} disponible — la vôtre a {days} jours de retard.",
  "claudeUpdate.important":
    "Sans cette mise à jour, Plume ne peut pas utiliser les modèles les plus récents.",
  "claudeUpdate.banner":
    "Une mise à jour importante de Claude Code est disponible ({installed} → {latest}). Sans elle, Plume ne peut pas utiliser les modèles les plus récents.",
  "claudeUpdate.failed": "La mise à jour de Claude Code a échoué : {message}",
  "claudeUpdate.install": "Mettre à jour",
  "claudeUpdate.updating": "Mise à jour de Claude Code…",
  "claudeUpdate.check": "Rechercher",
  "claudeUpdate.later": "Plus tard",
  "claudeUpdate.retry": "Réessayer",
  "updates.title": "Mises à jour",
  "updates.appName": "Plume {version}",
  "updates.unconfigured": "Les mises à jour ne sont pas configurées pour cette version.",
  "updates.checking": "Recherche en cours…",
  "updates.current": "Plume est à jour.",
  "updates.available": "Version {version} disponible.",
  "updates.availableAt": "Version {version} disponible — publiée le {date}.",
  "updates.installed": "Mise à jour installée — elle s'appliquera au redémarrage.",
  "updates.idle": "Dernière version connue de cette machine.",
  "updates.install": "Installer {version}",
  "updates.installing": "Installation…",
  "updates.restart": "Redémarrer maintenant",
  "updates.check": "Rechercher",
  "updates.downloading": "Téléchargement…",
  "updates.downloadingAt": "Téléchargement… {percent} %",
  "updates.auto": "Rechercher automatiquement au démarrage",

  // ---------------------------------------------------------------- console
  "console.title": "Journal technique",
  "console.shortcut": "⌘⌥C · Ctrl+Alt+C",
  "console.filter.all": "Tout",
  "console.filter.warnings": "Avertissements",
  "console.filter.errors": "Erreurs",
  "console.debug": "Détails techniques",
  "console.clear": "Vider",
  "console.empty": "Rien à signaler pour l'instant.",
  "console.lines.one": "{count} ligne",
  "console.lines.other": "{count} lignes",
  "console.scope.app": "app",
  "console.scope.claude": "claude",
  "console.scope.latex": "latex",
  "console.scope.workspace": "fichiers",
  "console.scope.render": "rendu",
  "console.scope.template": "charte",
  "console.scope.interface": "interface",
  "console.scope.ir": "ir",

  // ------------------------------------------------------------ advanced bar
  "advanced.banner": "MODE AVANCÉ",
  "advanced.banner.hint": "Les détails techniques sont visibles dans tout l'écran.",
  "advanced.model": "Modèle",
  "advanced.model.change": "Changer pour ce document",
  "advanced.cost": "Coût cumulé",
  "advanced.cost.meta.one": "{pages} · {count} passage",
  "advanced.cost.meta.other": "{pages} · {count} passages",
  "advanced.parallel": "Pages en parallèle",
  "advanced.parallel.auto": "(auto)",
  "advanced.doubt": "Seuil de doute",
  "advanced.doubt.meta.one": "{count} passage sous le seuil",
  "advanced.doubt.meta.other": "{count} passages sous le seuil",
  "advanced.openFolder": "Ouvrir le dossier du document",
  "advanced.openTex": "Voir le fichier .tex",

  // ----------------------------------------------------------------- wizard
  "wizard.title": "Nouveau document",
  "wizard.subtitle": "Étape {step} sur {total} — {name}",
  "wizard.step.title": "Le titre",
  "wizard.step.pages": "Les photos",
  "wizard.step.check": "Vérification",
  "wizard.title.label": "Titre du document",
  "wizard.title.placeholder": "Vecteurs",
  "wizard.tags.label": "Étiquettes",
  "wizard.tags.placeholder": "cours, exercices, DS…",
  "wizard.tags.hint": "Ce qu'est ce document, séparé par des virgules. Facultatif.",
  "wizard.template.label": "Mise en page",
  "wizard.template.meta.one": "{count} réglage · {engine}",
  "wizard.template.meta.other": "{count} réglages · {engine}",
  "wizard.drop.title": "Déposez vos photos ici",
  "wizard.drop.text": "ou {browse} · JPEG, PNG, HEIC",
  "wizard.drop.browse": "parcourez vos fichiers",
  "wizard.order.title.one": "{count} photo",
  "wizard.order.title.other":
    "{count} photos · glissez une ligne pour changer l'ordre des pages",
  "wizard.order.move.up": "Monter",
  "wizard.order.move.down": "Descendre",
  "wizard.order.remove": "Retirer",
  "wizard.ignored.one": "{count} fichier ignoré : seules les images sont acceptées.",
  "wizard.ignored.other": "{count} fichiers ignorés : seules les images sont acceptées.",
  "wizard.recap.title": "Titre",
  "wizard.recap.template": "Mise en page",
  "wizard.recap.pages": "Pages",
  "wizard.recap.first": "Première page",
  "wizard.footer.template": "Mise en page : {name} · modifiable plus tard",
  "wizard.create": "Créer le document",
  "wizard.creating": "Création…",
  "wizard.importing": "Import {done} / {total}…",

  // ------------------------------------------------------------ block kinds
  "kind.chapter": "Chapitre",
  "kind.part": "Partie",
  "kind.subpart": "Sous-partie",
  "kind.paragraph": "Paragraphe",
  "kind.text": "Texte",
  "kind.list": "Liste",
  "kind.equation": "Équation",
  "steps.adapt": "Aménagements",
  "adapt.side.title": "Adapter",
  "adapt.side.more": "D'autres aménagements viendront s'ajouter ici.",
  "adapt.gaps.name": "Texte à trous",
  "adapt.gaps.hint": "Masquer des mots que l'élève complétera",
  "adapt.gaps.how":
    "Surlignez les mots à masquer : ils laisseront place à des pointillés un peu plus larges qu'eux, où les écrire à la main. Glissez sur plusieurs mots pour n'ouvrir qu'un seul trou ; repassez dessus pour annuler.",
  "adapt.nothing":
    "Ce document n'a pas encore de passage à adapter. Les titres et les schémas ne se surlignent pas.",
  "adapt.count.one": "trou",
  "adapt.count.other": "trous",
  "adapt.clear": "Tout effacer",
  "export.pap.title": "Aménagement (PAP)",
  "export.pap.label": "Version aménagée",
  "export.pap.off": "Le document tel quel, sans aménagement.",
  "kind.definition": "Définition",
  "kind.vocabulary": "Vocabulaire",
  "kind.property": "Propriété",
  "kind.theorem": "Théorème",
  "kind.method": "Méthode",
  "kind.example": "Exemple",
  "kind.application": "Application",
  "kind.remark": "Remarque",
  "kind.proof": "Démonstration",
  "kind.figure": "Schéma",

  // -------------------------------------------------- triggers and effects
  "trigger.highlight": "Surligné",
  "trigger.marginBar": "Trait en marge",
  "trigger.underline": "Souligné à la main",
  "trigger.circled": "Entouré",
  "trigger.penColour": "Écrit à l'encre",
  "trigger.custom": "Autre marque",

  "effect.bold": "Mettre en gras",
  "effect.italic": "Mettre en italique",
  "effect.underline": "Souligner",
  "effect.teacherOnly": "Réserver à la version professeur",
  "effect.studentOnly": "Réserver à la version élève",
  "effect.blockKind": "Forcer le type de passage",
  "effect.skip": "Ne pas transcrire",
  "effect.custom": "Effet décrit à la main",

  "audience.teacher": "Professeur",
  "audience.student": "Élève",

  // ------------------------------------------------------------- documents
  "preview.tag.noted": "annoté",
  "preview.tag.teacher": "version prof",
  "preview.tag.student": "version élève",
  "preview.tag.layout": "mise en page",
  "preview.aside.teacher": "prof",
  "preview.aside.student": "élève",
  "preview.tag.hidden": "mis de côté",
  "preview.aside.hidden": "mis de côté — absent du PDF",
  "preview.hidden.restore": "remettre",
  "preview.layout.fallback":
    "Ce passage contient sa propre mise en page et le moteur n'a pas pu le rendre : l'aperçu l'empile en une colonne, le PDF la respectera.",
  "preview.stray.note":
    "Ce passage aligne sur un « & » sans environnement d'alignement autour. Le PDF ne compilera pas — corrigez le passage.",
  "preview.chapterLabel": "Chapitre {number} :",

  // ------------------------------------------------------------------ errors
  "error.refresh": "Rafraîchissement impossible",
  "error.load": "Chargement du classeur impossible",
} as const;
